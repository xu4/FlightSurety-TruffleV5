...
contract('Oracles', async (accounts) => {
  ...
  it('can request flight status', async () => {
    ...
    let insuranceAmount= web3.utils.toWei("0.1", "ether");
...
/*
  ...
  var eventWithdrawCredit = config.flightSuretyData.withdrawCredit();
    ...
    let status = await config.flightSuretyApp.fetchFlightStatus(config.firstAirline, flight, timestamp);
    ...
    let expectedCredit1 = web3.utils.toWei("0.15", "ether");
    assert.equal(creditBalance1, expectedCredit1, "credit for airline late status is wrong.");

    let expectedCredit2 = web3.utils.toWei("0.3", "ether");
